...
class Save extends \Magento\Backend\App\Action
{
    ...
    /**
     * @var \Akhil\Shop\Model\ResourceModel\Contact\CollectionFactory
     */
    ...
    public function __construct(
        Context $context,
        \Magento\Backend\Helper\Js $jsHelper,
        \Akhil\Shop\Model\ResourceModel\Contact\CollectionFactory $contactCollectionFactory
    ) {
    ...
    public function execute()
    {
        ...
        if ($data) {
            ...
            /** @var \Akhil\Shop\Model\Contact $model */
            ...
            try {
                ...
                $this->messageManager->addSuccess(__('You saved this shop.'));
                $this->_objectManager->get('Magento\Backend\Model\Session')->setFormData(false);
                if ($this->getRequest()->getParam('back')) {
                    return $resultRedirect->setPath('*/*/edit', ['shop_id' => $model->getId(), '_current' => true]);
                }
    ...
    public function saveProducts($model, $post)
    {
        ...
        if (isset($post['products'])) {
            ...
            try {
                ...
                $table = $this->_resources->getTableName(\Akhil\Shop\Model\ResourceModel\Contact::TBL_ATT_PRODUCT);
                $insert = array_diff($newProducts, $oldProducts);
                $delete = array_diff($oldProducts, $newProducts);

                if ($delete) {
                    $where = ['shop_id = ?' => (int)$model->getId(), 'product_id IN (?)' => $delete];
                ...
                if ($insert) {
                    ...
                    foreach ($insert as $product_id) {
                        $data[] = ['shop_id' => (int)$model->getId(), 'product_id' => (int)$product_id];
                    }
